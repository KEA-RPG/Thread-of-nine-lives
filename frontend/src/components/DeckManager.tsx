...
const DeckManager = ({ deckModel }: Props) => {
    ...
    const [deck, setDeck] = useState<Deck>(deckModel ?? {
        name: "",
        cards: [],
        isPublic: false,
        comments: [],
    ...
    const handleUpsert = async () => {
        try {
            ...
            const result = isNewDeck
                ? await postDeck(deck)
                : await putDeck(deck.id!, deck);
            ...
            <HStack spacing={25} mt={10} alignItems="flex-start">
                ...
                <SimpleGrid
                    ...
                    <VStack align="stretch">
                        <Button onClick={() => setDeck(prevDeck => ({ ...prevDeck, cards: [] }))} colorScheme='red'>Clear Deck</Button>
                        {deck.cards.length === 0 ? (
                            <Heading>No cards added</Heading>
                        ) : (
                            getGroupedDeck()
