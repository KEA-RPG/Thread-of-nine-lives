...
using (var scope = host.Services.CreateScope())
{
    ...
    await graphContext.InitCounter<GraphCard>(cards?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphComment>(comments?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphDeck>(decks?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphEnemy>(enemies?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphFight>(fights?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphGameAction>(gameActions?.Max(x => x.Id) ?? 1);
    await graphContext.InitCounter<GraphUser>(users?.Max(x => x.Id) ?? 1);
